// Uranus.js
let scene, camera, renderer, UranusMesh, ringsMesh;
...
function initVisualization() {
    ...
    const UranusTexture = textureLoader.load('UranusTexture.jpg'); // Replace with path to your texture
    
    // Uranus Rings
    const ringsTexture = textureLoader.load('UranusRingsTexture.jpg'); // Replace with path to your rings texture
    ...
    const material = new THREE.MeshPhongMaterial({ map: UranusTexture });
    UranusMesh = new THREE.Mesh(geometry, material);

    // Position the Uranus mesh
...
function animate() {
    ...
    // Rotate Uranus and its Rings
    UranusMesh.rotation.y += 0.005;
    ringsMesh.rotation.y += 0.005; // Rotate the rings at the same speed as Uranus
